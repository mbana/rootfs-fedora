...
function Restart-And-Resume([string] $script, [string] $step, [string] $command) 
{
	Restart-And-Run $global:restartKey "$global:powershell $script -Step $step -command $command"
}
...
if (Should-Run-Step "A") 
{
	...
	Restart-And-Resume $script "B" $command
}

if (Should-Run-Step "B") 
{
    start cmd "/k cd $ScriptDir & QCSecurityCheckTool.exe $command"
}